...
teardown() {
    ...
    unset HUAWEI_MODBUS_MQTT_BROKER
    unset HUAWEI_MODBUS_MQTT_PORT
    unset HUAWEI_MODBUS_MQTT_TOPIC
    unset HUAWEI_SLAVEID_AUTO
...
@test "MQTT broker from custom config" {
    ...
    [ "$HUAWEI_MODBUS_MQTT_BROKER" = "mqtt.custom.local" ]
}

@test "MQTT broker from HA service when no custom config" {
    source huawei_solar_modbus_mqtt/run.sh >/dev/null 2>&1

    [ "$HUAWEI_MODBUS_MQTT_BROKER" = "core-mosquitto" ]
}
...
@test "Environment variables are exported correctly" {
    ...
    [ "$HUAWEI_SLAVEID_AUTO" = "true" ]
    [ "$HUAWEI_SLAVE_ID" = "1" ]
    [ "$HUAWEI_MODBUS_MQTT_TOPIC" = "huawei-solar" ]
